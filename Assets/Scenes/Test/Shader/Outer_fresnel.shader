...
////// Emissive:
                ...
                float3 emissive = lerp(saturate(sceneColor.rgb),_Tint.rgb,saturate((_MainTex_var.rgb*(1.0 - pow(1.0-max(0,dot(normalDirection, viewDirection)),__FresnelAmount))*__Transparency)));
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float2 node_2186 = ((i.uv0+(_VerticalSpeed*node_3671.g)*float2(0,1))+(node_3671.g*_HorizontalSpeed)*float2(1,0));
                float4 _MainTex_var = tex2D(_MainTex,TRANSFORM_TEX(node_2186, _MainTex));
                o.Emission = lerp(saturate(sceneColor.rgb),_Tint.rgb,saturate((_MainTex_var.rgb*(1.0 - pow(1.0-max(0,dot(normalDirection, viewDirection)),__FresnelAmount))*__Transparency)));
