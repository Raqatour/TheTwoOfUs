...
Shader "Shader Forge/Inner_Fresnel" {
    Properties {
...
////// Lighting:
////// Emissive:
                float3 node_6669 = saturate(sceneColor.rgb);
                ...
                float node_2963 = pow(1.0-max(0,dot(normalDirection, viewDirection)),__FresnelAmount);
                float3 node_3140 = saturate((_MainTex_var.rgb*(1.0 - node_2963)*__Transparency));
                float3 emissive = lerp(node_6669,_Tint.rgb,node_3140);
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float node_2963 = pow(1.0-max(0,dot(normalDirection, viewDirection)),__FresnelAmount);
                float3 node_3140 = saturate((_MainTex_var.rgb*(1.0 - node_2963)*__Transparency));
                o.Emission = lerp(node_6669,_Tint.rgb,node_3140);
